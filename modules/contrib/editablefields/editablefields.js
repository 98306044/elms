...
Drupal.behaviors.editablefields = function(context) {
  ...
  //HACK to work with date popup
  $('input', context).not(':hidden').not('[name="field_due_date[0][value][date]"]').not('[name="field_end_date[0][value][date]"]').not('.colorpicker_textfield').focus();
  $('.colorpicker_textfield', context).not(':hidden').blur(function(){
	var rgbString = $(this).css('backgroundColor'); // get this in whatever way.
    ...
    var hexString = String('#'+ parts.join(''));
	$(this).val(hexString);
	$(this).change();
});
  //HACK to make field submit on esc
  /*$('input', context).not(':hidden').keyup(function(e) {
    if(document.all) {
			e = event;
		}
    if(e.keyCode==27){  // ESC/Enter pressed
	  $(this).change();  
    }
  });*/
