 ...
 */
define('elms_PROFILE_NAME', 'ELMS: e-Learning Management System');

/**
 * Description of profile; visible in profile selection form.
 */
define('elms_PROFILE_DESCRIPTION', 'This will install ELMS for the management of instructional content.');
...
function elms_profile_details() {
  return array(
    'name' => elms_PROFILE_NAME,
    'description' => elms_PROFILE_DESCRIPTION,
  );
}
 ...
 */
function elms_form_alter(&$form, $form_state, $form_id) {
  // Add an additional submit handler. 
  if ($form_id == 'install_configure') {
    $form['#submit'][] = 'elms_form_submit';
  }
}
 ...
 */
function elms_form_submit($form, &$form_state) {
  // Import database dump file.
  $elms_file = dirname(__FILE__) . '/elms.mysql';
  $success = import_elms($elms_file);
...
function import_elms($filename) {
  ...
  foreach (elms_list_tables() as $table) {
